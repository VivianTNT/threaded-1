# recsys/src/hm_only/user_modeling_rr.py
import pandas as pd
import numpy as np
import joblib
...
# ---- Load Artifacts ----
print("[user_modeling_rr] loading data ...")
events = pd.read_parquet(DATA / "events_rr.parquet")
# events_rr has offset user IDs (user_id >= 1_000_000_000)
...
style_path = ART / "user_style_embs_rr.joblib"
if style_path.exists():
    user_style_embs = joblib.load(style_path)
    print(f"[user_modeling_rr] loaded {len(user_style_embs)} user style embeddings")
else:
    user_style_embs = {}
    print("[user_modeling_rr] no user style embeddings found (skipping style fusion)")
...
def build_user_vectors_sparse(alpha=0.6):
    ...
    """
    print("[user_modeling_rr] building user vectors (sparse-matrix version)...")
    ...
    print(f"[user_modeling_rr] building sparse matrix with {len(unique_users):,} users, {X.shape[0]:,} items ...")
    R = sp.csr_matrix((vals, (rows, cols)), shape=(len(unique_users), X.shape[0]))

    print("[user_modeling_rr] multiplying R @ X ... (this may take a few minutes)")
    ...
    print(f"[user_modeling_rr] built {len(user_vecs):,} user vectors (with style fusion)")
    return user_vecs
